          ...
          lazy>
            <b-card-text class="gap--10px flex flex-column">
                ...
                <div class="flex flex-column row-container gap--10px">
                    <div class="flex flex-column custom-row gap--5px"
                     v-for="(row, rowIndex) in section.rows"
                     :key="rowIndex">
                        <div class="flex flex-row">
                            <h4 class="flex-grow-1">Lignes n° {{ rowIndex + 1 }}</h4>
                            <DeleteAreYouSure
                              :i="`are-you-sure-${rowIndex}`"
                              @sure="deleteLine(sectionIndex, rowIndex)"
                            />
                        </div>
                        ...
                        <div
                          ...
                          class="flex flex-row gap--5px">
                            ...
                            <b-button
                              variant="danger"
                              @click="deleteAttribute(sectionIndex, rowIndex, attributeIndex)">
                                <b-icon-trash-fill />
                            </b-button>
                        </div>
                        <div class="flex justify-center">
                            <b-button
                              @click="addAttributeToRow(row, sectionIndex, rowIndex)">
                              Add attributes
                            </b-button>
                        </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    sumAttributesSizeRow(attributes) {
      return attributes.reduce((accumulator, attribute) => accumulator + parseInt(attribute.size, 10), 0);
    },
    saveCharacterSheet() {
      const sections = [...this.sections];
      const data = { ...this.characterSheetBuilding, sections };
